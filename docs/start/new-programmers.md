...
1. Complete [workstation setup](/start/workstation-setup) and build the project without editing it.
...
7. Test the change without hardware when possible, then follow the team’s robot test process.
...
A beginner change is complete when it compiles, the relevant test passes, the expected telemetry is visible, failure behavior is understood, and another programmer can explain the change from the diff.
